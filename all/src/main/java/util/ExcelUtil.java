...
public class ExcelUtil {
    ...
    public static XSSFWorkbook createExcel(List<String> filedNames, List<Map<String, Object>> rowValues) {
        int cellIndex = 0;
        int rowNumIndex = 0;
        XSSFWorkbook wb = new XSSFWorkbook();
        XSSFSheet sheet = wb.createSheet();

        XSSFRow row = sheet.createRow(rowNumIndex);
        if (CollectionUtils.isEmpty(filedNames) || CollectionUtils.isEmpty(rowValues)) {
            return wb;
        }
        ...
        for (Map<String, Object> map : rowValues) {
            rowNumIndex++;
            cellIndex = 0;
